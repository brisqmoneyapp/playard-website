...
function getTimeLeft(eventTimeMs: number, now: number): TimeLeft | null {
  const difference = eventTimeMs - now;
...
export default function EventCountdown({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const next = getTimeLeft(eventTimeMs, Date.now());
    ...
    <section className="border-y-4 border-black bg-black px-4 py-12 text-white sm:px-6 sm:py-16">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className={`mb-3 text-sm font-black uppercase tracking-[0.25em] ${accentClassName}`}>
              Coming Up
            </p>
            <h2 className="text-4xl font-black uppercase leading-none tracking-[-0.07em] text-white sm:text-5xl md:text-6xl">
              {title}
            </h2>
            <p className={`mt-3 text-lg font-bold sm:text-xl ${accentClassName}`}>{dateLine}</p>
          </div>
        ...
        {mounted && hasStarted ? (
          <div className="border-4 border-[#00d4ff] bg-[#111111] p-8 text-center shadow-2xl sm:p-10">
